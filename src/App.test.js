...
jest.mock('./api/fetchShow'); //{returns a new Promise()}
...
test('component renders without errors', async () => {
  ...
  render(<App />);
  //Assert poster, title, summary, dropdown
  expect(await screen.findByAltText(/stranger things/i)).toBeInTheDocument();
  expect(
    await screen.findByRole('heading', { name: /stranger things/i })
  ).toBeInTheDocument();
  expect(await screen.findByTestId(/summary/i)).toBeInTheDocument();
