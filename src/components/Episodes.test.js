...
test('rerender episodes with correct data & length', () => {
  const { rerender } = render(<Episodes episodes={[]} />);

  expect(screen.queryAllByText(/chapter/i)).toHaveLength(0);

  rerender(<Episodes episodes={mockEpisodeData} />);

  expect(screen.getAllByText(/chapter/i)).toHaveLength(mockEpisodeData.length);
});
